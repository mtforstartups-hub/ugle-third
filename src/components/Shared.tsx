...
export const showToast = (msg: string) => window.dispatchEvent(new CustomEvent('show-toast', {detail: msg}));
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-start gap-12 mb-16 md:mb-20">
          <div className="w-full md:max-w-sm">
            ...
            <p className="mt-6 text-gray-400 font-medium leading-relaxed">
              The local-first search application for media professionals. Find the soundbite. Pull the clip. Hit the deadline.
            </p>
            <div className="mt-8 flex gap-6 text-sm font-semibold text-gray-400">
              ...
              <Link to="/download" className="hover:text-white transition-colors text-[#75C043]">Download</Link>
            </div>
          ...
          <div className="w-full md:w-auto flex flex-col md:items-end relative min-h-[6rem]">
            <label className="block font-mono text-xs text-[#75C043] mb-3 tracking-widest uppercase font-semibold">Join the Waitlist</label>
            <AnimatePresence mode="wait">
              {submitted ? (
                ...
                >
                  ...
                  <span className="font-mono text-xs uppercase tracking-widest text-[#75C043] font-bold">Successfully Joined</span>
                </motion.div>
              ) : (
                <motion.form
                  key="form"
                ...
                >
                  <div className="flex flex-col sm:flex-row w-full gap-2 sm:gap-0 relative">
                    ...
                    />
                    <button type="submit" className="bg-[#75C043] hover:bg-[#86d950] text-ugle-slate font-mono text-xs font-bold px-8 py-3 rounded sm:rounded-l-none transition-all shadow-[0_0_15px_rgba(117,192,67,0.3)] w-full sm:w-auto whitespace-nowrap uppercase tracking-wider hover:shadow-[0_0_25px_rgba(117,192,67,0.6)] focus:outline-none focus:ring-2 focus:ring-[#75C043] focus:ring-offset-2 focus:ring-offset-[#1C1C1C]">
                      Join
                    </button>
                  </div>
                  <AnimatePresence>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-6">
          <div className="flex gap-4">
            <Link to="/privacy" className="text-gray-400 hover:text-white transition-colors font-mono text-xs">Privacy Policy</Link>
            <Link to="/terms" className="text-gray-400 hover:text-white transition-colors font-mono text-xs">Terms of Service</Link>
          </div>
          ...
          <div className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors relative group justify-center sm:justify-end">
            ...
            <select className="bg-transparent border-none outline-none cursor-pointer appearance-none font-mono text-xs focus:outline-none uppercase tracking-wider pr-6 z-10 relative">
              <option value="en" className="text-ugle-slate">EN - English</option>
              <option value="es" className="text-ugle-slate">ES - Español</option>
              <option value="fr" className="text-ugle-slate">FR - Français</option>
              <option value="de" className="text-ugle-slate">DE - Deutsch</option>
              <option value="ja" className="text-ugle-slate">JA - 日本語</option>
            </select>
